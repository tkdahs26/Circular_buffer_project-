...
실시간으로 모니터링에 용이한 프로그램으로 제한된 크기의 배열에 데이터가 바뀔때마다 평균, 최대, 최소가 실시간으로 업데이트되는 시스템입니다
...
순서를 나열한 begin() 실행시 배열 순서와 상관없이 오래 입력 순으로 나열되기 때문에 가장 오래된 24.5가 삭제되고 두번째입력한 25.2가 첫번째 인덱스로 이동했습니다 
